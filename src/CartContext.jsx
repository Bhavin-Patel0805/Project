...
export function CartProvider({ children }) {
    ...
    //to icrease value of item in cart
    ...
    //to decrease the value of 0 item remove from cart

    const decrement = (id) => {
        setCart((prev) =>
            prev
                .map((p) => (p.id === id ? { ...p, qty: p.qty - 1 } : p))
                .filter((p) => p.qty > 0) // remove the item id qty =0
        );
    };
    ...
    }; // check the number is right correct for sum of not
